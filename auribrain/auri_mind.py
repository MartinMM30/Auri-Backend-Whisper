...
class AuriMind:
  ...
  def __init__(self):
    ...
    self.actions = ActionsEngine()   # ⭐ Nuevo
  ...
  def think(self, user_msg: str):
    ...
    # 7) ActionsEngine (puede generar acción y frase final distinta)
    act_result = self.actions.handle(
      intent=intent,
      user_msg=user_msg,
      context=ctx,
      memory=self.memory,
    ...
    final_from_actions = act_result.get("final")

    if final_from_actions:
      final_answer = final_from_actions
    else:
      # 8) procesado final normal (sin acciones especiales)
